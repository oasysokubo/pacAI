...
class QLearningAgent(ReinforcementAgent):
    ...
    def getValue(self, state):
        ...
        if legalMoves.isTerminal(state):
            return 0.0
        else:
            maxQVal = max([self.getQValue(state, action) for action in legalMoves])
        return maxQVal
    ...
    def getPolicy(self, state):
        ...
        if legalMoves.isTerminal(state):
            return None
        values = counter.Counter()  # A Counter is a dict with default 0
